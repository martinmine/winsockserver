...
void ConnectionManager::connectionAccepted()
{
	...
	{									// Try and get a free connection
		IncommingConnection* connection = getFreeConnection();
		...
		if (wouldNotBlock)				// Ignore if pending IO
		{
			if (connection->valid())	// If a connection was received
			{
				eventListener->connectionAccepted(	// Cast it
					reinterpret_cast<IncommingConnection*>(connection));
...
IncommingConnection* ConnectionManager::getFreeConnection()
{
	IncommingConnection* sess = nullptr;
	...
	if (sess == nullptr)	// No free connections, make one
		sess = new IncommingConnection(connectionsAccepted++, this);

	return sess;
}

void ConnectionManager::onDisconnect(NetworkConnection* connection)
{
	IncommingConnection* conn = reinterpret_cast<IncommingConnection*>(connection);
	eventListener->connectionDisconnected(conn);	// Notify
	conn->setFree();					// Set it to be free
	freeConnections.push_front(conn);	// Add to 'free' connections
}

void ConnectionManager::registerListener(IClientConnectionEventListener* lstnr)
{
	eventListener = lstnr;
}
